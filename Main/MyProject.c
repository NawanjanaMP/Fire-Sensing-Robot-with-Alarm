...
void StartSignal(){
 TRISD.RD7 = 0; //Configure RD7 as output
 PORTD.RD7 = 0; //RD0 sends 0 to the sensor
 delay_ms(18);
 PORTD.RD7 = 1; //RD0 sends 1 to the sensor
 delay_us(30);
 TRISD.RD7 = 1; //Configure RD0 as input
 }
...
char ReadData(){
 char i, j;
 for(j = 0; j < 8; j++){
 while(!PORTD.RD7); //Wait until PORTD.F0 goes HIGH
 delay_us(30);
 if(PORTD.RD7 == 0)
 i&= ~(1<<(7 - j)); //Clear bit (7-b)
 else {i|= (1 << (7 - j)); //Set bit (7-b)
 while(PORTD.RD7);} //Wait until PORTD.F0 goes LOW
 }
 return i;
 }
...
TRISD.RD7 = 0;
 ...
 //TRISD.B5 = 0; // Set  DATA_pin as output pin
 //TRISD.B4 = 0; // Set  CLCOK_pin as output pin
 //TRISD.B6 = 0; // Set LATCH_pin as output pin
...
TRISC.RC4 = 1;
TRISC.RC7 = 1;
//OSCCON = 0x76;   configure OSCCON to use 8MHz internal OScillator
